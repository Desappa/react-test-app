...
$(document).ready(function () {
  ...
  setTimeout(() => {
    var scrollTo_int = $('li a.active').first().offset().top;
    if (scrollTo_int > window.innerHeight) {
      $('.slimscroll').slimScroll({ scrollTo: scrollTo_int - 100 + 'px' });
    }
  }, 300)
});
...
function showNotification(msg) {
  ...
  notification.innerHTML = msg ? msg : "Form submitted successfully!";
...
function pdfUrlToBase64(pdfUrl) {
  return fetch(pdfUrl)
    .then((response) => {
      if (!response.ok) {
    ...
    .then((buffer) => {
      // Convert the ArrayBuffer to a base64 string in chunks
      let binary = "";
